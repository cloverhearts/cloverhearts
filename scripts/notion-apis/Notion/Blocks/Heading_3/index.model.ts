...
class Heading3BlockItem implements BlockItem {
  ...
  toHTML(nestedBlocksHTML: string = ""): string {
    ...
    return `<h3 class="${this.attributes.join(
      " "
...
export const Heading3Block = {
  build(notionBlock: BlockItem | any): BlockItem {
    if (NotionBlock.instanceOfBlockItem(notionBlock)) {
      return new Heading3BlockItem(notionBlock);
    }
    ...
    return new Heading3BlockItem(blockItem.build());
  },
};
