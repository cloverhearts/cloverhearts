...
class NumberedListItem implements BlockItem {
  ...
  get type(): string {
    return BLOCK_TYPES.NUMBERED_LIST_ITEM;
  }
  ...
  toHTML(nestedBlocksHTML: string = ""): string {
    ...
    return `<ol><li class="${this.attributes.join(
      " "
...
export const NumberedListItemBlock = {
  build(notionBlock: BlockItem | any): BlockItem {
    if (NotionBlock.instanceOfBlockItem(notionBlock)) {
      return new NumberedListItem(notionBlock);
    }
    console.log("START---- ", notionBlock);
    ...
    return new NumberedListItem(blockItem.build());
  },
};
