...
class Heading2BlockItem implements BlockItem {
  ...
  toHTML(nestedBlocksHTML: string = ""): string {
    ...
    return `<h2 class="${this.attributes.join(
      " "
...
export const Heading2Block = {
  build(notionBlock: BlockItem | any): BlockItem {
    if (NotionBlock.instanceOfBlockItem(notionBlock)) {
      return new Heading2BlockItem(notionBlock);
    }
    ...
    return new Heading2BlockItem(blockItem.build());
  },
};
